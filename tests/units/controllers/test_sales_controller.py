...
class TestSalesController:
    def test_given_valid_sales_params_when_query_sales_by_employee_then_return_correct_sales_data(self, mocker, data_fixture):
        mocker.patch.object(DatamartController, 'load_data', return_value=data_fixture)
    ...
    def test_given_invalid_sales_params_when_query_sales_by_employee_then_return_none_sales_data(self, mocker, data_fixture):
        mocker.patch.object(DatamartController, 'load_data', return_value=data_fixture)
        data_controller = SalesController(datamart_model=DatamartController())
        response = data_controller.get_sales_by_key_employee(params=SalesQueryParams(start_date=date(2023, 11, 1),
                                                                                     end_date=date(2024, 1, 31),
